...
class SingleProductLazadaSpider(scrapy.Spider):
    name = 'single-product-lazada'
    id_product = None
    ...
    def get_API_by_itemId_and_totalReview(self, urlProduct):
        ...
        try:
            ...
            product['total_rating'] = response_data['model']['ratings']['average']
    ...
    def convertDatetime(self, date):
        try:
            if date.find("tuần trước"):
                ...
                format_date = "{}/{}/{}".format(date.split(" ")[0], date.split(" ")[2], date.split(" ")[3])
    ...
    def __init__(self, urlProduct=None, *args, **kwargs):
        super(SingleProductLazadaSpider, self).__init__(*args, **kwargs)
        self.start_urls = self.get_API_by_itemId_and_totalReview(urlProduct)
...
def startProcess(urlProduct):
    ...
    job = Job(SingleProductLazadaSpider, urlProduct=urlProduct)
    processor.run(job)
